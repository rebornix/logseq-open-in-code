<template>
  ...
  >
    <div class="container-inner shadow-lg" v-if="ready" :style="{left: left+'px', top: top+'px'}">
        <div class="_opener" @click="_onClickOpenCurrentPage">Edit current page</div>
        ...
        <div class="_opener" @click="_onClickOpenCurrentGraph">Open graph folder</div>
    </div>
  </div>
</template>
...
function openPageInVSCode() {
    logseq.Editor.getCurrentPage().then(currentPage => {
        if (currentPage && currentPage.file) {
            ...
            logseq.DB.datascriptQuery(`[:find ?file
                :where
                [?b :file/path ?file]
                [(== ?b ${fileId})]
            ]`).then(matches => {
  ...
  mounted () {
    ...
    logseq.on('ui:visible:changed', ({ visible }) => {
      if (visible) {
        ...
        const el = top.document.querySelector(`div[data-injected-ui=open-in-code--${key}]`)
